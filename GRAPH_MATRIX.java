public class GRAPH_MATRIX{
    ...
    public void KanteLoeschen(String Beginn, String Ende){                
        // Kante darf nicht gelöscht werden, wenn Beginn == Ende 
        if(Beginn != Ende){
            // Nummern der betreffenden Kanten suchen
            int Beg = KnotenNummer(Beginn);
            int End = KnotenNummer(Ende);
            
            // Wenn beide Knoten existieren, Kante entfernen
            if((Beg != -1) && (End != -1)){
                Matrix [Beg][End] = -1;
                Matrix [End][Beg] = -1;
            }        
    ...
    public String KnotenEntfernen_2(String Kno){ //$ Letzer Knoten außerhalb der Schleife
        int KnoNum = KnotenNummer(Kno); // holt Knotennummer     
        if(KnoNum != -1){   // Knoten existiert    ?     
            // Geht die Zeilen nach und nach durch bis auf die Letzte
            for(int i = KnoNum; i < AnzahlKnoten - 1; i++){             
                // Ändert den Knotenkopf
                System.out.println("Entferne Knoten  " + i + " " + Kno);
                Knoten[i] = Knoten[i + 1]; //hab da erst mit KnoNum gearbeitet aber i erhöht smh // funktioniert
                // Geht pro Zeile alle Spalten durch bis auf die Letzte
                for(int j = 0; j < KnoNum-1; j++){
                ...
                // letzte Spalte/Zeile: Kante entfernen
            ...
            Knoten[AnzahlKnoten - 1 ] = null; //Wusste erst nich wie ich an die letzte Arraystelle komme
            AnzahlKnoten--;
            return Kno; //vllt die Lösung idk?
        }else{
